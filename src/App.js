...
class App extends Component {
  state = {videosList: initialTracksList, searchInput: ''}
  ...
  onClickDelete = id => {
    ...
    const updatedList = videosList.filter(eachItem => eachItem.id !== id)
    this.setState({videosList: updatedList})
  }

  render() {
    const {videosList, searchInput} = this.state
    const finalMusicList = videosList.filter(eachItem =>
    ...
    return (
      <div className="main-container">
        ...
        <div className="second-part">
          ...
          {finalMusicList.length !== 0 ? (
            <ul className="playlist-container">
              {finalMusicList.map(eachItem => (
                <MusicList
